...
def yield_func_kim_van(sigma1_in, sigma2_in, alpha, beta, k_pow_6):
    """
    Evaluates the Kim-Van yield function f = J₂³ + α J₃² + β J₂^(3/2) J₃ - k⁶ = 0
    ...
    try:
        ...
        term1 = np.power(j2_safe, 3)
        term2 = alpha * np.power(j3, 2)
        ...
        if abs(beta) > 1e-12: # Use tolerance for float comparison
             ...
             term3 = beta * np.power(j2_safer_for_pow, 1.5) * j3
        else:
             term3 = 0.0 # If beta is zero, this term is zero

        f = term1 + term2 + term3 - k_pow_6
...
def calculate_locus_data(alpha, beta, k, grid_bound_factor, n_points):
    ...
    k_pow_6 = k**6; grid_bound = grid_bound_factor * k
    ...
    try:
        f_values_2d = yield_func_kim_van(s1_grid_2d, s2_grid_2d, alpha, beta, k_pow_6)
